...
SET 
  ...
  description = 'Join us in celebrating ElaJa''s special day with joy, laughter, and wonderful memories! Share your wishes, upload photos, and take the fun quiz.',
  target_date = '2025-12-25T00:00:00+00:00'  -- Update this to ElaJa's actual birthday
WHERE title = 'Birthday Celebration';

-- Clear existing quiz questions and add personalized ones for ElaJa
DELETE FROM quiz_questions WHERE celebration_id IN (
  SELECT id FROM celebrations WHERE title = 'ElaJa''s Amazing Birthday Celebration'
);
...
DECLARE
  celebration_id uuid;
BEGIN
  SELECT id INTO celebration_id FROM celebrations WHERE title = 'ElaJa''s Amazing Birthday Celebration' LIMIT 1;
